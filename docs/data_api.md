...
- `train_epoch_pickled()`: The main training function. This trains for an entire epoch. There is a function nested inside this one to train for one batch. Note the following parameters:
    ...
    - `n_subsample_targets`: how many points to randomly subsample in the target set. This may be good normalization, and was used in lots of my experiments. Leaving this `None` disables target subsampling.
...
`subsample_targets()`: Used to subsample a random set of values from the target set. This is called within the training loop if `n_subsample_targets` is set. This could be moved into the deepsensor repository as it is quite useful + might provide really good normalization.
